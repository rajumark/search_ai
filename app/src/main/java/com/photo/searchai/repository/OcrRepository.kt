...
/**
 * Repository for OCR and image processing operations. Handles syncing MediaStore images to Room and
 * triggering processing work.
 */
interface OcrRepository {
    /** Sync images from MediaStore to Room database. Adds new images and removes deleted ones. */
...
constructor(
    ...
    override suspend fun syncImagesFromMediaStore() {
        ...
        val faceParsed = imageDao.getFaceParsedCountFlow().first()
        val facePending = total - faceParsed
    ...
    override fun getFaceParsedCountFlow(): Flow<Int> = imageDao.getFaceParsedCountFlow()
    override fun getFacePendingCountFlow(): Flow<Int> = imageDao.getFacePendingCountFlow()
    ...
    override suspend fun enqueueOcrWorkIfNeeded() {
        ...
        if (ocrPending > 0 || barcodePending > 0 || labelPending > 0 || facePending > 0) {
    ...
    override suspend fun getSearchSuggestions(query: String): List<String> {
        ...
        val queryTokens =
                query.lowercase().split("\\s+".toRegex()).filter { it.isNotEmpty() }.toSet()
        ...
        return tokenCounts.entries.sortedByDescending { it.value }.take(suggestionLimit).map {
            it.key
        }
    }
}
